...
def test_check_dz_db():
    ...
    for _ in range(20):
        p = np.random.randint(2, 20)
        x = np.random.random(p).reshape(-1, 1)
        w = np.random.random(p).reshape(-1, 1)
...
def test_check_dL_dw():
    ...
    for _ in range(20):
        p = np.random.randint(2, 20)
        x = (2 * np.random.random(p) - 1).reshape(-1, 1)
        y = np.random.randint(0, 2)
        w = (2 * np.random.random(p) - 1).reshape(-1, 1)
        b = 2 * np.random.random(1) - 1
...
def test_check_dL_db():
    ...
    for _ in range(20):
        p = np.random.randint(2,20)
        x = np.random.random((p,1)).reshape(-1, 1)
        w = np.random.random((p,1)).reshape(-1, 1)
        b = np.random.random(1)
        y = np.random.randint(0,2) 
        z, a, L= forward(x,y,w,b)
        dL_da, da_dz, dz_dw, dz_db = backward(x,y,a)
...
def test_update_w():
    '''(4 point) update_w'''
    w = np.array([0.0, 0.0])
    dL_dw = np.array([1.0, 2.0])


    w = update_w(w,dL_dw, alpha=.5) 
    
    w_true = -np.array([0.5, 1.0])
...
def test_predict():
    ...
    Y_true = [0, 1, 1, 1]
    P_true = np.array([0.401312339887548, 0.6681877721681662, 0.5, 0.52497918747894]).reshape(-1, 1)
